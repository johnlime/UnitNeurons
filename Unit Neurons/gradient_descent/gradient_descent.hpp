...
class FloatFeedForwardNeuron: FloatUnitNeuron{
protected:
...
public:
    ...
    FloatFeedForwardNeuron(FloatUnitNeuron** _prevs, int _num_prevs, const std::string _activ);
    void feedforward();
...
private:
    FloatFeedForwardNeuron** targets;   // feed forward neurons in the order of update
    int num_targets;
    int* layer_sizes;
    int num_layers;
    float loss;
...
public:
    ...
    void calculate_l1_loss(float correct_value, float target_output);
    void execute();
};
